...
def get_relevant_context(input, vault_embeddings, vault_content, top_k=3):
    if vault_embeddings.nelement() == 0:  # Check if the tensor has any elements
        return []
    # Encode the rewritten input
    input_embedding = ollama.embeddings(model='nomic-embed-text', prompt=input)["embedding"]
...
def ollama_chat(user_input, system_message, vault_embeddings, vault_content, ollama_model, conversation_history):
    ...
    response=""
    for response_chunk in client.chat(model=ollama_model,messages=messages,stream=True):
        print(response_chunk['message']['content'],end='', flush=True)
        response+=response_chunk['message']['content']
